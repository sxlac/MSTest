@fobt
@envnot=prod
Feature: FOBT Exam Performed - Billing
...
@TestCaseKey=ANC-T346
Scenario Outline: FOBT Performed - Sent to Billing
...
@TestCaseKey=ANC-T729
Scenario Outline: FOBT Performed - Sent to Billing - Track BillRequestSent
        * def randomBarcode = Faker().randomDigit(6)
        * def eventId = DataGen().uuid()
        * set evaluation.answers =
            """
        ...
        * match examStatusResults[*].FOBTStatusCodeId contains 1 && 8 && 12 && 13
        ...
        # # # # Commenting out the publish to topic since T-Checks automatically publishes this event
        ...
        * def billAcceptedResult = FobtDb().getBillingResultsByEvaluationId(evaluation.evaluationId)[0]
...
@TestCaseKey=ANC-T349
Scenario Outline: FOBT Performed - Undetermined Test Results - BillRequestNotSent
    * def randomBarcode = Faker().randomDigit(6)
    * def eventId = DataGen().uuid()
    * set evaluation.answers =
        """
    ...
    * match resultsEvent.PerformedDate contains  dateStamp
    ...
    * match resultsEvent.IsBillable.toString() ==  'false'
    * match resultsEvent.Result[0].Result ==  <lab_result>
    * match resultsEvent.Result[0].Exception ==  <exception_message>
