...
namespace Signify.HBA1CPOC.Svc.Core.Tests;
...
public class FakeTransactionSupplier : ITransactionSupplier, IFakeTransactionSupplier
{
    ...
    private sealed class Transaction : IBufferedTransaction
    {
        ...
        private volatile bool _didExplicitlyRollback;
        ...
        public Task CommitAsync(CancellationToken cancellationToken = new())
        {
            lock (_locker)
            {
                ...
                if (_didExplicitlyRollback)
                    Throw("Attempted to commit after transaction was rolled back");
        ...
        public Task RollbackAsync(CancellationToken cancellationToken = new())
        {
            lock (_locker)
            {
                ...
                if (_didExplicitlyRollback)
                    Throw("Attempted to rollback the transaction more than once");

                _didExplicitlyRollback = true;
