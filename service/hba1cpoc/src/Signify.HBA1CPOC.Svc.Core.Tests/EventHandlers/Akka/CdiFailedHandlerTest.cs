...
public class CdiFailedHandlerTest
{
    private readonly TestableEndpointInstance _endpointInstance;
    private readonly CdiFailedEventHandler _cdiFailedHandler;
    private readonly CDIFailedEvent _cdiFailedEvent;

    public CdiFailedHandlerTest()
    {
        _cdiFailedEvent = new CDIFailedEvent()
        {
        ...
        _cdiFailedHandler = new CdiFailedEventHandler(A.Dummy<ILogger<CdiFailedEventHandler>>(), _endpointInstance);
    }
    ...
    public async Task EvaluationFinalizedHandler_WhenProductCodeCorrect(string productCode, int valid)
    {
        ...
        _cdiFailedEvent.Products = new List<DpsProduct>
            { new DpsProduct(12345, productCode), new DpsProduct(12345, "PAD") };

        await _cdiFailedHandler.Handle(_cdiFailedEvent, CancellationToken.None);
