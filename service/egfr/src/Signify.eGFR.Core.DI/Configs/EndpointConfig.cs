...
public static class EndpointConfig
{
    public static EndpointConfiguration Create(IConfiguration config)
    {
        ...
        recoverability.Immediate(immediate => { immediate.NumberOfRetries(nsbConfig.ImmediateRetryCount); });
        ...
        persistence.TablePrefix("eGFR_");

        var connectionString = config.GetConnectionString(ConnectionStringNames.eGFRDatabase);
    ...
    private static void ConfigureTransport(TransportDefinition transport, ServiceBusConfig nsbConfig,
        EndpointConfiguration endpointConfiguration)
    {
        endpointConfiguration.UseTransport(transport)
            .RouteToEndpoint(Assembly.GetAssembly(typeof(EvalReceivedHandler)), nsbConfig.QueueName);
