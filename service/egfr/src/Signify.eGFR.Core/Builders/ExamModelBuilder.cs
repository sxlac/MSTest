...
public class ExamModelBuilder : IExamModelBuilder
{
    ...
    private readonly IVendorDetermination _vendorDetermination;
        ...
        long providerId)
    {
        ...
        _answers = answers;
        _vendorDetermination = vendorDetermination;
    ...
    public IExamModelBuilder ForEvaluation(long evaluationId)
    {
        ...
        return new ExamModelBuilder(_logger,
            evaluationId,
            _answers != null ? new Dictionary<int, EvaluationAnswerModel>(_answers) : null, _vendorDetermination, _formVersionId, _providerId);
    }
    ...
    public IExamModelBuilder WithFormVersion(int formVersionId)
    {
        ...
        return new ExamModelBuilder(_logger, _evaluationId, _answers, _vendorDetermination, formVersionId, _providerId);
    }
    ...
    public IExamModelBuilder WithAnswers(IEnumerable<EvaluationAnswerModel> answers)
    {
        ...
        {
            // At least for eGFR, we don't have any Q's that can be answered more than once
        ...
        return new ExamModelBuilder(_logger, _evaluationId, dict, _vendorDetermination, _formVersionId, _providerId);
    }
    ...
    public IExamModelBuilder WithProviderId(long providerId)
    {
        ...
        return new ExamModelBuilder(_logger, _evaluationId, _answers, _vendorDetermination, _formVersionId, providerId);
    }
    ...
    public ExamModel Build()
    {
        ...
        if (TryGetOptional(KedTestPerformedQuestion.QuestionId, out var kedTestPerformedQ))
        {
            // Check for KED question ids
        ...
        // In this case it should always have the KED question or it's a bug
        else if (_formVersionId >= 595)
        {
            throw new RequiredEvaluationQuestionMissingException(_evaluationId, _formVersionId, KedTestPerformedQuestion.QuestionId);
        }

        //Check for valid eGFR question ids
        var eGfrTestPerformedQ = GetRequired(TestPerformedQuestion.QuestionId);

        switch (eGfrTestPerformedQ.AnswerId)
        {
            ...
            default:
                _logger.LogWarning("eGFR not yet configured to process not performed evals");
                break;
        }
    ...
    private NotPerformedReason ParseNotPerformed(int questionId)
    {
        ...
        return q.AnswerId switch
        {
            ...
            KedReasonNotPerformedQuestion.ProviderUnableToPerform => GetUnableToPerformResult(KedReasonProviderUnableToPerformQuestion.QuestionId),
            ReasonNotPerformedQuestion.MemberRefusedAnswerId => GetMemberRefusedResult(ReasonMemberRefusedQuestion.QuestionId),
            ReasonNotPerformedQuestion.ProviderUnableToPerform => GetUnableToPerformResult(ReasonProviderUnableToPerformQuestion.QuestionId),
            ReasonNotPerformedQuestion.ClinicallyNotRelevant => NotPerformedReason.ClinicallyNotRelevant,
    ...
    private NotPerformedReason GetUnableToPerformResult(int questionId)
    {
    ...
    private bool TryGetOptional(int questionId, out EvaluationAnswerModel answerModel)
        => _answers.TryGetValue(questionId, out answerModel);
    ...
    {
        return !string.IsNullOrEmpty(numericBarcode) && Regex.IsMatch(numericBarcode, "^LGC([-]{1}[0-9]{4}){3}$", RegexOptions.IgnoreCase);
    }
