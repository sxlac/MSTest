    ...
    : IRequestHandler<UpdateExamStatus>
{
    ...
    [Trace]
    private async Task<AddExamStatusResponse> SaveStatusToDb(ExamStatusEvent message, CancellationToken cancellationToken)
    {
        ...
        (
            ...
            {
                ExamStatusCodeId = message.StatusCode.StatusCodeId,
                StatusDateTime = message.StatusDateTime.ToUniversalTime(), // The event time could be in timezone other than UTC
    ...
    {
        switch (message.StatusCode.StatusCodeId)
        {
    ...
    /// <param name="cancellationToken"></param>
    [Trace]
    private async Task PublishStatusToKafka(Guid eventId, BaseStatusMessage eventToPublish, CancellationToken cancellationToken)
    {
    ...
    {
        switch (statusMessage.StatusCode.StatusCodeId)
        {
            case (int)StatusCode.CdiPassedReceived:
            case (int)StatusCode.CdiFailedWithPayReceived:
            case (int)StatusCode.CdiFailedWithoutPayReceived:
