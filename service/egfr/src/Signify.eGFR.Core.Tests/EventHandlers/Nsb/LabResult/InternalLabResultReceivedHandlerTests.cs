...
public sealed class InternalLabResultReceivedHandlerTests
{
    ...
    public async Task Handle_LabResultReceivedEvent_HappyPath()
    {
        ...
        var kedEgfrLabResult = new KedEgfrLabResult
        {
            EvaluationId = 123,
            EgfrResult = 60.45m,
        ...
        A.CallTo(() => _mapper.Map<KedEgfrLabResult>(A<JsonElement>._)).Returns(kedEgfrLabResult);

        // Act
        await CreateSubject().Handle(labResultReceivedEvent, _fakeContext);

        // Assert
        A.CallTo(() => _mapper.Map<KedEgfrLabResult>(A<JsonElement>._)).MustHaveHappenedOnceExactly();
