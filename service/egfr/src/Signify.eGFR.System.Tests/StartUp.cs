...
public class StartUp
{
    ...
    private static void NewRelicSetup(string env, IConfigurationRoot config, TestContext testContext)
    {
        ...
        var newRelicConfig = new NewRelicConfig
        {
        ...
        BaseTestActions.NewRelicActions = new NewRelicActions(newRelicConfig, TestConstants.LoggingHttpMessageHandler);
    }
    ...
    private static async Task KafkaSetup(string env, IConfiguration config, TestContext testContext)
    {
        ...
        var kafkaHaProducerConfig = KafkaUtils.GetKafkaProducerConfig(env,producerHaConfig);
        BaseTestActions.CoreKafkaActions = new CoreKafkaActions(kafkaConfig, kafkaProducerConfig);
        BaseTestActions.CoreHomeAccessKafkaActions = new CoreKafkaActions(kafkaConfig, kafkaHaProducerConfig);
    ...
    private static void DatabaseSetup(string env, IConfiguration config, TestContext testContext)
    {
        ...
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("EGFR_DB_HOST")))
            Environment.SetEnvironmentVariable("EGFR_DB_HOST", config.GetValue<string>($"{env}:databases:egfr:host"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("EGFR_DB_USERNAME")))
            Environment.SetEnvironmentVariable("EGFR_DB_USERNAME",
                config.GetValue<string>($"{env}:databases:egfr:username"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("EGFR_DB_PASSWORD")))
            Environment.SetEnvironmentVariable("EGFR_DB_PASSWORD",
                config.GetValue<string>($"{env}:databases:egfr:password"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("SERVICE")))
            Environment.SetEnvironmentVariable("SERVICE", config.GetValue<string>($"{env}:databases:egfr:name"));
...
private static async Task CoreApiSetup(string env, IConfiguration config, TestContext testContext)
    ...
    private static async Task StartKafkaConsumer(string env,KafkaConfig kafkaConfig, TestContext testContext)
    {
        ...
        if (!BaseTestActions.CoreKafkaActions.IsConsumerReady())
        {
