...
public class StartUp
{
    ...
    private static void DatabaseConfig(string env, IConfiguration config, TestContext testContext)
    {
        ...
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("DEE_DB_HOST")))
            Environment.SetEnvironmentVariable("DEE_DB_HOST", config.GetValue<string>($"{env}:databases:dee:host"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("DEE_DB_USERNAME")))
            Environment.SetEnvironmentVariable("DEE_DB_USERNAME",
                config.GetValue<string>($"{env}:databases:dee:username"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("DEE_DB_PASSWORD")))
            Environment.SetEnvironmentVariable("DEE_DB_PASSWORD",
                config.GetValue<string>($"{env}:databases:dee:password"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("SERVICE")))
            Environment.SetEnvironmentVariable("SERVICE", config.GetValue<string>($"{env}:databases:dee:name"));
