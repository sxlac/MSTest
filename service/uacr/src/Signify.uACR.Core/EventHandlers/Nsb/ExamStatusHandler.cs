    ...
    : IHandleMessages<ExamStatusEvent>
{
    ...
    {
        logger.LogDebug("Start {Handle} for EvaluationId={EvaluationId}", nameof(ExamStatusHandler),
        ...
        logger.LogDebug("End {Handle} for EvaluationId={EvaluationId}", nameof(ExamStatusHandler),
            message.EvaluationId);
    }

    private async Task<Exam> GetExam(long evaluationId, CancellationToken token)
    {
        return await mediator.Send(new QueryExamByEvaluation { EvaluationId = evaluationId }, token);
    }
    ...
    private async Task<BaseStatusMessage> CreateKafkaStatusEvent(ExamStatusCode statusCode, Exam exam,
        ExamStatusEvent message, CancellationToken token)
    {
        switch (statusCode.ExamStatusCodeId)
        {
    ...
    private string GetReasonType(int answerId)
    {
        switch (answerId)
        {
            ...
            default:
                logger.LogWarning(
                    "Unable to match AnswerId={AnswerId} with set Member Refusal and Unable to Perform reason types",
                    answerId);
                return string.Empty;
        }
    }
    ...
    private void EmitNewRelicAttributes(ExamStatusCode status)
    {
        ...
        switch (status.ExamStatusCodeId)
        {
            case (int) StatusCode.ExamPerformed:
