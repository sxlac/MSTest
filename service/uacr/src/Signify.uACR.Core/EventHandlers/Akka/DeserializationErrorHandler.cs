    ...
    : IStreamingNotificationHandler<DeserializationError>
{
    ...
    /// on message type.
    /// </summary>
    private BaseDlqMessage GenerateDlqMessage(DeserializationError notification)
    {
        var topics = kafkaTopics.Topics;
        
        var topicToDlqTypeMapping = new Dictionary<string, Func<BaseDlqMessage>>
        {
            { topics["Evaluation"], () => new EvaluationDlqMessage() },
            { topics["PDF delivered to client"], () => new PdfDeliveryDlqMessage() },
            { topics["CDI Events"], () => new CdiEventDlqMessage() },
            { topics["Bill Accepted"], () => new RcmBillDlqMessage() },
            { topics["Lab Results"], () => new DpsLabResultDlqMessage() },
            { topics["RMS Labresult"], () => new DpsRmsLabResultDlqMessage() }
        };
