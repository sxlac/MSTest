...
public class InternalLabResultHandlerTests
{
    ...
    private InternalLabResultHandler CreateSubject() => new(_logger, _productFilter, _messageSession, _publishObservability, _featureFlags);

    [Fact]
    public async Task Handle_LabResult_CallToNsbOccured_HappyPath()
    {
        const int expectedLabResultId = 32;
        var expectedProductCodes = new[]{ "EGFR", "UACR" };
        var e = new LabResultReceivedEvent
        {
            LabResultId = 32,
            ProductCodes = ["EGFR", "UACR"]
        };
