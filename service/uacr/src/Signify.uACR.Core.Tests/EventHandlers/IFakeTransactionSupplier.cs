...
namespace Signify.uACR.Core.Tests.EventHandlers;
...
public class FakeTransactionSupplier : ITransactionSupplier, IFakeTransactionSupplier
{
    ...
    public void AssertCommit()
    {
        ...
        Assert.False(_transaction.DidThrow,
            "Code under test had an exception thrown when using a transaction, and the exception did not bubble up to the test");
    ...
    private sealed class Transaction : IBufferedTransaction
    {
        ...
        private volatile bool
            _didThrow; // In case the SUT throws and catches an exception that isn't raised to the test
