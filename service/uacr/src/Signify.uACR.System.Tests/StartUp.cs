...
using Dps.Test.Utilities.Wiremock.Constants;
...
using Signify.QE.Core.Utilities.NewRelic.Config;
...
public class StartUp
{
    ...
    public static async Task TestStartup(TestContext testContext)
    {
        ...
        NewRelicSetup(env, config, testContext);
        WiremockSetup(env, config);
        WebhookApiSetup(env, config, testContext);
    }

    private static void LaunchDarklySetup(string env, IConfigurationRoot config)
    {
        if (env.Equals("local")) return;
        TestConstants.LaunchDarklyConfig = new LaunchDarklyConfig
        {
            SdkKey =  Environment.GetEnvironmentVariable("LD_SDK_KEY") ??
                      config.GetValue<string>($"{env}:launchDarkly:sdkKey"),
            UserKey = Environment.GetEnvironmentVariable("LD_USER_KEY") ??
                      config.GetValue<string>($"{env}:launchDarkly:userKey"),
            UserName = Environment.GetEnvironmentVariable("LD_USERNAME") ??
    ...
    private static async Task KafkaSetup(string env, IConfiguration config, TestContext testContext)
    {
        ...
        var lhaProducerConfig = new KafkaConfig
        {
        ...
        var resultsProducerConfig = KafkaUtils.GetKafkaProducerConfig(env,lhaProducerConfig);
        
        BaseTestActions.CoreKafkaActions = new CoreKafkaActions(kafkaConfig, kafkaProducerConfig);
        BaseTestActions.LhaKafkaActions = new CoreKafkaActions(kafkaConfig, resultsProducerConfig);
    ...
    private static void DatabaseConfig(string env, IConfiguration config, TestContext testContext)
    {
        ...
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("UACR_DB_HOST")))
            Environment.SetEnvironmentVariable("UACR_DB_HOST", config.GetValue<string>($"{env}:databases:uacr:host"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("UACR_DB_USERNAME")))
            Environment.SetEnvironmentVariable("UACR_DB_USERNAME",
                config.GetValue<string>($"{env}:databases:uacr:username"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("UACR_DB_PASSWORD")))
            Environment.SetEnvironmentVariable("UACR_DB_PASSWORD",
                config.GetValue<string>($"{env}:databases:uacr:password"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("SERVICE")))
            Environment.SetEnvironmentVariable("SERVICE", config.GetValue<string>($"{env}:databases:uacr:name"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("OMS_DB_HOST")))
            Environment.SetEnvironmentVariable("OMS_DB_HOST", config.GetValue<string>($"{env}:databases:oms:host"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("OMS_DB_USERNAME")))
            Environment.SetEnvironmentVariable("OMS_DB_USERNAME",
                config.GetValue<string>($"{env}:databases:oms:username"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("OMS_DB_PASSWORD")))
            Environment.SetEnvironmentVariable("OMS_DB_PASSWORD",
                config.GetValue<string>($"{env}:databases:oms:password"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("ILR_DB_HOST")))
            Environment.SetEnvironmentVariable("ILR_DB_HOST", config.GetValue<string>($"{env}:databases:internallabresultapi:host"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("ILR_DB_USERNAME")))
            Environment.SetEnvironmentVariable("ILR_DB_USERNAME",
                config.GetValue<string>($"{env}:databases:internallabresultapi:username"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("ILR_DB_PASSWORD")))
            Environment.SetEnvironmentVariable("ILR_DB_PASSWORD",
                config.GetValue<string>($"{env}:databases:internallabresultapi:password"));
        testContext.WriteLine("Database environment variables set.");
    }

    private static void SmbSetup(string env, IConfiguration config, TestContext testContext)
    {
        if (env.Equals("prod")) return;
        testContext.WriteLine("Setting up Smb environment variables.");
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("SMB_URL")))
            Environment.SetEnvironmentVariable("SMB_URL", config.GetValue<string>($"{env}:smb:url"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("SMB_USERNAME")))
            Environment.SetEnvironmentVariable("SMB_USERNAME", config.GetValue<string>($"{env}:smb:username"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("SMB_PASSWORD")))
            Environment.SetEnvironmentVariable("SMB_PASSWORD", config.GetValue<string>($"{env}:smb:password"));
    }
    ...
    private static void WiremockSetup(string env, IConfigurationRoot config)
    {
        WiremockUrl = env.Equals("local") ? WiremockLocalBaseUrl : Constants.WiremockUrl;
        
        BaseTestActions.WiremockActions =
            new WiremockActions(WiremockUrl,TestConstants.LoggingHttpMessageHandler);
        
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("LGC_CLIENT_ID")))
            Environment.SetEnvironmentVariable("LGC_CLIENT_ID",
                config.GetValue<string>($"{env}:wiremock:lgc:client_id"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("LGC_CLIENT_SECRET")))
            Environment.SetEnvironmentVariable("LGC_CLIENT_SECRET",
                config.GetValue<string>($"{env}:wiremock:lgc:client_secret"));
    }

    private static void NewRelicSetup(string env, IConfigurationRoot config, TestContext testContext)
    {
        if (env.Equals("prod")) return;
        testContext.WriteLine("Setting up NewRelic Api config");
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("NEWRELIC_ACCOUNT_ID")))
            Environment.SetEnvironmentVariable("NEWRELIC_ACCOUNT_ID",
                config.GetValue<string>($"newrelic:accountId"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("NEWRELIC_API_KEY")))
            Environment.SetEnvironmentVariable("NEWRELIC_API_KEY", config.GetValue<string>($"newrelic:apiKey"));
        var nRConfig = new NewRelicConfig
        {
            AccountId = Environment.GetEnvironmentVariable("NEWRELIC_ACCOUNT_ID"),
            ApiKey = Environment.GetEnvironmentVariable("NEWRELIC_API_KEY"),
            Environment = env,
            RetryCount = 20,
            RetryDelay = 3000
        };
        BaseTestActions.NewRelicActions = new NewRelicActions(nRConfig, TestConstants.LoggingHttpMessageHandler);
    }
    
    private static void WebhookApiSetup(string env, IConfigurationRoot config, TestContext testContext)
    {
        var labsWebhookApiUrl = testContext.Properties["LabsWebhookApiUrl"]!.ToString();
        
        BaseTestActions.WebhookApiActions = new WebhookApiActions(labsWebhookApiUrl, TestConstants.LoggingHttpMessageHandler);
        
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("API_SECRET_KEY_LGC")))
            Environment.SetEnvironmentVariable("API_SECRET_KEY_LGC",
                config.GetValue<string>($"{env}:secretKeyLGC"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("API_SECRET_KEY_TESTVENDOR")))
            Environment.SetEnvironmentVariable("API_SECRET_KEY_TESTVENDOR",
                config.GetValue<string>($"{env}:secretKeyTestVendor"));
    }
}