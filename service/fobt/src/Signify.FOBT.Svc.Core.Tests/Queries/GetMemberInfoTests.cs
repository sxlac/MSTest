...
public class GetMemberInfoTest
{
    ...
    private readonly GetMemberInfoHandler _handler;
    ...
    public GetMemberInfoTest()
    {
        ...
        _handler = new GetMemberInfoHandler(logger, _memberApi, _mapper);
    }
    ...
    {
        var providerInfo = new GetMemberInfo { MemberPlanId = 21074095 };
        A.CallTo(() => _memberApi.GetMemberInfoById(A<int>._)).Returns(new ApiResponse<MemberInfoRs>(
            new HttpResponseMessage { StatusCode = System.Net.HttpStatusCode.BadRequest }, new MemberInfoRs(), new RefitSettings()));
        A.CallTo(() => _mapper.Map<MemberInfoRs>(A<MemberInfoRs>._)).Returns(GetMappedMemberResponse());
        await Assert.ThrowsAsync<ApplicationException>(async () => await _handler.Handle(providerInfo, CancellationToken.None));
    }
