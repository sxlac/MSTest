...
namespace Signify.FOBT.Svc.Core.NotificationHandlers
{
    /// <summary>
    /// Handler for handling <see cref="StreamingError"/> events raised by Akka, signaling the registered handler
    /// for the consumable message failed to process the event within the configured maximum backoff retries.
    /// </summary>
    ...
    public class StreamingErrorHandler : IStreamingNotificationHandler<StreamingError>
    {
        ...
        public Task Handle(StreamingError notification, CancellationToken cancellationToken)
        {
            _logger.LogError(notification.Exception,
                "Error streaming message on Topic={Topic} at Offset={Offset} on Partition={Partition}, DeserializedMessage: {DeserializedMessage}",
