...
namespace Signify.PAD.Svc.Core.Behaviors
{
    public class MediatrUnitOfWork<TRequest, TResponse> : IPipelineBehavior<TRequest, TResponse>
    {
        ...
        public MediatrUnitOfWork(ILogger<MediatrUnitOfWork<TRequest, TResponse>> logger, PADDataContext context, IMessageProducer producer)
        {
            _logger = logger;
            _context = context;
            _producer = producer;
