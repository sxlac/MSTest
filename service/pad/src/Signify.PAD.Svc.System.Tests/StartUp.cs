...
{
    private static void Log(TestContext context, string msg)
    {
        context.WriteLine($"[{DateTime.Now:HH:mm:ss}] {msg}");
    }
    ...
    private static async Task KafkaSetup(string env, IConfiguration config, TestContext testContext)
    {
        ...
        Log(testContext, "Setting up Kafka config");
        ...
        TestConstants.KafkaConfig = new()
        {
            ...
            GetEventRetryCount = 30 * (1_000 / intervalMs), // Continue retrying for up to 30s
            GetEventRetrySleep = intervalMs
        };
        ...
        Log(testContext, "Kafka setup complete.");
    }

    private static void DatabaseSetup(string env, IConfiguration config, TestContext testContext)
    {
        Log(testContext, "Setting up database environment variables");
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("PAD_DB_HOST")))
            Environment.SetEnvironmentVariable("PAD_DB_HOST", config.GetValue<string>($"{env}:databases:pad:host"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("PAD_DB_USERNAME")))
            Environment.SetEnvironmentVariable("PAD_DB_USERNAME",
                config.GetValue<string>($"{env}:databases:pad:username"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("PAD_DB_PASSWORD")))
            Environment.SetEnvironmentVariable("PAD_DB_PASSWORD",
                config.GetValue<string>($"{env}:databases:pad:password"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("SERVICE")))
            Environment.SetEnvironmentVariable("SERVICE", config.GetValue<string>($"{env}:databases:pad:name"));
        
        Log(testContext, "Database environment variables set");
    }

    private static void SmbSetup(string env, IConfiguration config, TestContext testContext)
    {
        if (env.Equals("prod")) return;
        Log(testContext, "Setting up SMB environment variables.");
    ...
    {
        if (env.Equals("prod")) return;
        Log(testContext, $"Starting Kafka consumer, subscribing to {kafkaConfig.Topics.Length} topics: {string.Join(',', kafkaConfig.Topics)}");
    ...
    private static async Task CreateProvider(string env, TestContext testContext)
    {
        switch (env)
        {
            ...
            case "prod":
                ...
                break;
            default:
                Log(testContext, "Creating test provider");
                ...
                Log(testContext, "Test provider created");
                break;
        }
    }
}