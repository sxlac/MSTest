...
public class StartUp
{
    ...
    private static void DatabaseSetup(string env, IConfiguration config, TestContext testContext)
    {
        ...
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("SPIROMETRY_DB_HOST")))
            Environment.SetEnvironmentVariable("SPIROMETRY_DB_HOST", config.GetValue<string>($"{env}:databases:spirometry:host"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("SPIROMETRY_DB_USERNAME")))
            Environment.SetEnvironmentVariable("SPIROMETRY_DB_USERNAME",
                config.GetValue<string>($"{env}:databases:spirometry:username"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("SPIROMETRY_DB_PASSWORD")))
            Environment.SetEnvironmentVariable("SPIROMETRY_DB_PASSWORD",
                config.GetValue<string>($"{env}:databases:spirometry:password"));
        if (string.IsNullOrEmpty(Environment.GetEnvironmentVariable("SERVICE")))
            Environment.SetEnvironmentVariable("SERVICE", config.GetValue<string>($"{env}:databases:spirometry:name")); 
